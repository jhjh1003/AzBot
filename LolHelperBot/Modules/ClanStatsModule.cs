...
// /전적수집(운영자 전용)으로 등록 멤버들의 매치를 모아 저장하고, 나머지 명령은 저장된 데이터를 조회만 합니다.
// /리플업로드는 .rofl 리플레이 파일에서 직접 전적을 추출합니다 (Riot API 없이 검증용/보조 수집 경로).
...
public class ClanStatsModule : InteractionModuleBase<SocketInteractionContext>
{
    ...
    // 최소인원 파라미터를 없애면서 상수로 고정. /전적수집·/리플업로드가 애초에 5명 전원인 경기만
        ...
        foreach (var account in accountsToScan)
        {
            ...
            foreach (var matchId in newIds)
            {
                ...
                foreach (var team in qualifyingTeams)
                {
                    ...
                    // /부캐충돌목록·/부캐충돌해결로 실제 플레이어를 지정할 수 있게 합니다.
                    ...
                    if (ownerGroups.Count > 0)
                    {
                        ...
                        ownerCollisionWarnings.Add(
                            ...
                            "(같은 사람으로 등록된 계정 2개가 한 팀에 동시에 있었음 — 부캐를 다른 사람이 썼을 가능성. `/부캐충돌목록` 참고)");
    ...
    public async Task ShowOwnerConflictsAsync()
    {
        ...
        var embed = new EmbedBuilder()
            ...
            .WithFooter("/부캐충돌해결 매치아이디: 롤아이디: 멤버: 로 각 계정의 실제 플레이어를 지정해 주세요.")
            .Build();
    ...
    public async Task ResolveOwnerConflictAsync(
        [Summary("매치아이디", "/부캐충돌목록에 나온 매치 ID (예: KR_8286962111)")]
        string matchId,
        ...
        IUser member)
    {
        ...
        {
            await FollowupAsync(
                "❌ 해당 매치·롤아이디의 미해결 충돌 기록을 찾지 못했습니다. `/부캐충돌목록`으로 정확한 값을 확인해 주세요.",
                ephemeral: true);
            return;
        }
    ...
    [SlashCommand("리플업로드", ".rofl 리플레이 파일을 분석해 전적 데이터를 미리 보거나 저장합니다 (기본은 미리보기만, 저장 안 함).")]
    public async Task UploadReplayAsync(
        [Summary("리플파일", ".rofl 리플레이 파일")]
        IAttachment replayFile,
        [Summary("저장", "true로 지정하면 실제로 통계 DB에 저장합니다 (기본 false = 미리보기만)")]
        bool save = false,
        ...
        IUser? reassignMember = null)
    {
        ...
        if (save && !Context.CanManageMembers())
        {
            await FollowupAsync(
                "❌ 실제 저장은 서버 소유자와 서버 관리 권한이 있는 운영자만 할 수 있습니다. (`저장:false`로 미리보기는 누구나 가능)",
                ephemeral: true);
            return;
        }
        ...
        var embedBuilder = new EmbedBuilder()
            .WithTitle(savedThisRun ? "✅ 리플 데이터 저장 완료" : "🔍 리플 데이터 미리보기 (저장 안 함)")
            ...
            .WithFooter(savedThisRun
                ? "⚠️ 이미 /전적수집(Riot API)으로 같은 경기를 모았다면 중복 집계될 수 있습니다."
    ...
    public async Task ShowWinRateRankingAsync()
    {
        ...
        var embed = new EmbedBuilder()
            ...
            .WithDescription(string.Join("\n", lines))
            .WithFooter($"운영자가 /전적수집 을 실행한 시점까지의 데이터 기준입니다. 최소 {MinSampleSize}판 이상 (표본 부족 시 전체 표시)")
            .Build();

        await FollowupAsync(embed: embed);
    }

    [SlashCommand("내전적", "우리 클랜 데이터 기준 자유 랭크 총 승률·라인별 승률·모스트/워스트 챔피언을 보여줍니다.")]
        ...
        IUser? member = null)
    {
        ...
        if (positionRows.Count == 0)
        {
            ...
            await FollowupAsync(
                $"{who} 저장된 자유 랭크 전적이 없습니다. AtoZ 등록 여부와 `/전적수집`(또는 `/리플업로드`) 실행 여부를 확인해 주세요.");
            return;
        }
    ...
    [SlashCommand("조합추천", "우리 클랜 전적 데이터 기준, 같은 팀에서 함께 나왔을 때 승률이 좋은 챔피언 조합을 보여줍니다.")]
    public async Task ShowDuoRecommendationAsync()
    {
        ...
        if (rows.Count == 0)
        {
            await FollowupAsync("같은 팀으로 함께 플레이한 자유 랭크 전적이 아직 없습니다. 운영자가 `/전적수집`을 먼저 실행해야 해요.");
            return;
        }
    ...
    [SlashCommand("바텀듀오", "원딜+서폿 멤버 조합별 승률을 모스트 10 / 워스트 10으로 보여줍니다.")]
    public async Task ShowBottomDuoStatsAsync()
    {
        ...
        if (rows.Count == 0)
        {
            await FollowupAsync("같은 팀 원딜+서폿으로 함께한 자유 랭크 전적이 아직 없습니다. 운영자가 `/전적수집`을 먼저 실행해야 해요.");
            return;
        }
    ...
    [SlashCommand("봇듀오챔프승률", "원딜+서폿 챔피언 조합별 승률을 모스트 10 / 워스트 10으로 보여줍니다.")]
    public Task ShowBotDuoChampionStatsAsync() =>
        ShowLaneChampionDuoAsync("AtoZ 봇(원딜+서폿) 챔피언 조합 승률", "BOTTOM", "UTILITY", "원딜", "서폿");

    [SlashCommand("정글미드듀오챔프승률", "정글+미드 챔피언 조합별 승률을 모스트 10 / 워스트 10으로 보여줍니다.")]
    ...
    private async Task ShowLaneChampionDuoAsync(string title, string positionA, string positionB, string labelA, string labelB)
    {
        ...
        if (rows.Count == 0)
        {
            await FollowupAsync($"같은 팀 {labelA}+{labelB}으로 함께한 자유 랭크 전적이 아직 없습니다. 운영자가 `/전적수집`을 먼저 실행해야 해요.");
            return;
        }
    ...
    [SlashCommand("아재전적", "AtoZ 멤버 5명 전원이 한 팀으로 5인큐한 자유 랭크 경기 기록만 최근 순으로 보여줍니다.")]
        ...
        int count = 5)
    {
        ...
        if (matches.Count == 0)
        {
            await FollowupAsync(
                ...
                "운영자가 `/전적수집` 또는 `/리플업로드`로 데이터를 먼저 모아야 해요.");
            return;
        }
    ...
    [SlashCommand("명예의전당", "이번 달(또는 지정한 연월)의 기여도 베스트 플레이어 랭킹을 보여줍니다.")]
        ...
        string? 연월 = null)
    {
        ...
        if (gamesScored.Count == 0)
        {
            await FollowupAsync(
                ...
                "`/전적수집`으로 모은 경기만 집계됩니다 — 리플 업로드로만 저장된 경기는 지표가 없어서 제외됩니다.");
            return;
        }
        ...
        var embed = new EmbedBuilder()
            ...
            .WithFooter(
                ...
                "매달 새로 집계됩니다(누적 아님). `/전적수집`으로 모은 경기만 집계됩니다.")
            .Build();
