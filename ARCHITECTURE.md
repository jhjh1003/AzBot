...
| 파일 | 역할 | 알아두면 좋은 점 |
|---|---|---|
| **ClanStatsModule.cs** | 이 봇의 핵심. `/전적수집`, `/티어픽`, `/밴픽추천`, `/아재전적`, `/명예의전당`, `/내전적`, `/조합추천` 등 클랜 통계 관련 명령어 대부분이 여기 모여 있음 | 지금 1577줄로 제일 큰 파일. 커맨드가 늘어날수록 계속 커지는 중이라, 계산 로직을 `Services/`로 옮기는 작업(리팩토링)을 하나씩 진행 중 |
...
- **2단계 (진행 중, 1호 완료):** 커맨드 핸들러 안에 섞여 있던 계산 로직을 서비스로 하나씩
  뽑아내는 작업. `/밴픽추천` → `BanPickRecommendationService`를 첫 사례로 만들었고, 앞으로
  다른 명령(`/티어픽`, `/명예의전당` 등)도 같은 패턴으로 옮길 예정.
...
| `/전적수집 [최근경기수]` | Riot API에서 경기 데이터 모아서 DB에 저장 |
| `/전적등록후보` | 자주 같이 한 미등록 계정 찾기 |
| `/부캐충돌목록` / `/부캐충돌해결` | 부캐 중복 사용으로 누락된 데이터 복구 |
| `/리플업로드 저장:true` | `.rofl` 데이터 실제 저장 |
