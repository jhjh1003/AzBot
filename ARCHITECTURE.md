...
(프로젝트 루트, LolHelperBot/ 밖)
├── Dockerfile                 ← 이 봇을 실행 가능한 "상자"로 포장하는 방법 (10절)
└── .github/workflows/         ← push할 때마다 그 상자를 자동으로 만들어주는 로봇 (10절)
```
...
| 파일 | 역할 | 알아두면 좋은 점 |
|---|---|---|
| **ClanStatsModule.cs** | 이 봇의 핵심. 운영자 기능은 부모 `/atoz` 그룹 아래에 두고, `/티어픽`, `/밴픽추천`, `/아재전적`, `/명예의전당`, `/내전적`, `/조합추천` 같은 조회 명령은 최상위로 노출 | 커맨드가 늘어날수록 계속 커지는 중이라 계산 로직을 `Services/`로 옮기는 작업을 하나씩 진행 중. "그 판 팀 5명 중 몇 위 기여를 했나"를 계산하는 로직(`ComputeContributionRanks`)은 `/아재전적`과 `/내전적`(최근 전적 섹션)이 공유함(2026-08-22 중복 제거) |
...
| **MatchRepository.cs** | 경기 기록 DB 테이블을 읽고 씀. 이 봇의 거의 모든 통계 쿼리(`/티어픽`, `/승률순위`, `/조합추천`...)가 결국 이 파일의 메서드를 호출함. 제일 크고 제일 중요한 파일. 저장된 경기의 소유자를 재배정하는 기능(`ReassignParticipationOwnerAsync`, 2026-08-22 — 아래 8절 부캐 관련 설명 참고)과, 특정 멤버의 최근 경기 목록을 팀 전체와 함께 가져오는 기능(`GetRecentMemberMatchesAsync`)도 여기 있음 |
...
## 10. 이 봇은 실제로 어디서 돌아가나 (배포 구조, 2026-08-22)

처음엔 만든 사람의 개인 PC에서 `dotnet run`으로만 돌아갔습니다(그래서 PC를 끄면 봇도 죽었음).
지금은 클라우드에서 24시간 돌아갑니다. 비유하면 **"직접 요리하던 개인 주방"에서 "본사가 미리
포장해서 보내주는 프랜차이즈 매장"**으로 바뀐 셈입니다.

| 프랜차이즈 비유 | 실제 | 하는 일 |
|---|---|---|
| **레시피북** | `Dockerfile` | 코드를 어떻게 컴파일해서 실행 가능한 "상자"(컨테이너 이미지)로 포장할지 정의 |
| **본사 포장 공장** | GitHub Actions (`.github/workflows/docker-publish.yml`) | `main`에 push될 때마다 자동으로 그 레시피대로 상자를 만듦(컴파일) |
| **물류 창고** | GHCR(GitHub Container Registry) | 완성된 상자(이미지)가 쌓이는 곳 |
| **실제 매장** | Vultr Cloud VPS(서울 리전) | Docker로 창고에서 상자를 받아와 실행만 함 — **매장은 절대 요리(빌드)를 하지 않음** |
| **매장 냉장고** | Docker 볼륨(`azbot-data`) | SQLite DB 파일이 여기 영속화됨. 매장을 리모델링(컨테이너 재생성)해도 냉장고 내용물은 안 사라짐 |

**왜 "포장"과 "매장"을 분리했나**: 처음엔 클라우드 VM 위에서 직접 `docker build`(코드
컴파일)까지 하려고 했는데, 이 봇을 **실행**하는 덴 100~200MB면 충분해도 **컴파일**하는 순간엔
1GB 넘게 필요해서, 저사양 무료 VM에서 빌드가 메모리 부족으로 계속 죽었습니다. 그래서 "사양
좋은 GitHub 서버가 미리 다 포장해두고, VM은 완성품만 받아 쓰는" 구조로 분리했습니다. 배포를
처음부터 다시 하는 스텝바이스텝 가이드는 [DEPLOY.md](DEPLOY.md) 참고.

## 11. 이 봇이 제공하는 명령어 전체 목록
...
| `/승률순위` | 클랜원 전체 승률 순위 |
| `/내전적 [멤버] [월]` | 개인 승률/최근 전적(2026-08-22 추가)/라인별 통계/모스트·워스트 챔피언 |
...
| `/atoz 전적재배정` (2026-08-22 추가) | 저장은 정상적으로 됐지만, **그 경기만 다른 사람이 부캐를 빌려서 한 경우** 기록 소유자를 옮김(부캐충돌과는 다른 케이스) |
